#!/usr/bin/env python3
"""
Core Neo4j data loading logic (TSV -> Neo4j).

Ported from the legacy icdc-dataloader repo's data_loader.py, adapted to use
this repo's existing os_loader_icdc_schema.py (ICDC_Schema, is_parent_pointer).
"""
...
from os_loader_icdc_schema import ICDC_Schema, is_parent_pointer
...
def backup_neo4j(backup_dir, name, address, log):
    try:
        restore_cmd = (
            "To restore DB from backup (to remove any changes caused by current data loading, run "
            "following commands:\n "
        )
        restore_cmd += "#" * 160 + "\n"
        neo4j_cmd = "neo4j-admin restore --from={}/{} --force".format(backup_dir, name)
        mkdir_cmd = ["mkdir", "-p", backup_dir]
        is_shell = False
        ...
        cmds = [
            mkdir_cmd,
            ["neo4j-admin", "backup", "--backup-dir={}".format(backup_dir)],
        ]
        if address in ["localhost", "127.0.0.1"]:
            ...
            second_cmd = "sudo systemctl stop neo4j && {} && sudo systemctl start neo4j && exit".format(
                neo4j_cmd
            )
...
class DataLoader:
                    ...
                    with open(txt, encoding=file_encoding) as in_file:
                        ...
                        for org_obj in reader:
                            ...
                            if id_field not in obj.keys():
                                self.log.error(
                                    f"Line: {line_number}: Required id field {id_field} is missing, validation failed"
                                )
                                return False
                            elif obj[id_field] is None:
                                self.log.error(
                                    f"Line: {line_number}: Required id field {id_field} is None, validation failed"
                                )
                                return False
                            if NODE_TYPE not in obj.keys():
                                self.log.error(
                                    f"Line: {line_number}: Required node type field {NODE_TYPE} is missing, validation failed"
                                )
                                return True
                            elif obj[NODE_TYPE] is None:
                                self.log.error(
                                    f"Line: {line_number}: Required node type field {NODE_TYPE} is None, validation failed"
                                )
                                return False
                            node_type = obj.get(NODE_TYPE, None)
                            if not self.node_exists(
                                session, node_type, id_field, obj[id_field]
                            ):
                                self.log.error(
                                    f'Line: {line_number}: The node to be deleted (:{obj[NODE_TYPE]} {{{id_field}: "{obj[id_field]}"}}) not found in DB!, validation failed'
                                )
    ...
    def validate_files(
        self, cheat_mode, loading_mode, file_list, max_violations, temp_folder, verbose
    ):
        if not cheat_mode:
            if loading_mode != DELETE_MODE:
                ...
                if validation_failed:
                    ...
                    df_validation_result_file_key = os.path.basename(
                        os.path.dirname(file_list[0])
                    )
                    timestamp = get_time_stamp()
                    output_key_invalid = (
                        os.path.join(temp_folder, df_validation_result_file_key)
                        + "_"
                        + timestamp
                        + ".xlsx"
                    )
                    ...
                    for key in self.df_validation_dict.keys():
                        sheet_name_new = key
                        self.df_validation_dict[key].to_excel(
                            writer, sheet_name=sheet_name_new, index=False
                        )
                    writer.close()
    ...
    def load(
        ...
        if not self.validate_files(
            cheat_mode, loading_mode, file_list, max_violations, temp_folder, verbose
        ):
            return False
        ...
        if dry_run:
            ...
            self.log.info("Dry run mode, no nodes or relationships loaded.")
            self.log.info("Running time: {:.2f} seconds".format(end - start))
        ...
        self.log.info(
            "{} nodes and {} relationships loaded!".format(
                self.nodes_created, self.relationships_created
            )
        )
        self.log.info(
            "{} nodes and {} relationships deleted!".format(
                self.nodes_deleted, self.relationships_deleted
            )
        )
        self.log.info(
            "{} nodes updated!".format(self.nodes_updated, self.relationships_deleted)
        )
        self.log.info("Loading time: {:.2f} seconds".format(end - start))
        return {
            NODES_CREATED: self.nodes_created,
            RELATIONSHIP_CREATED: self.relationships_created,
            NODES_DELETED: self.nodes_deleted,
            RELATIONSHIP_DELETED: self.relationships_deleted,
    ...
    def cleanup_node(node):
        return {
            key if not key else key.strip(): value if not value else value.strip()
    ...
    def prepare_node(self, node, file_name):
        ...
        if node_type:
                ...
                if key_type == "Boolean":
                    ...
                    if isinstance(value, str):
                        if re.search(r"yes|true", value, re.IGNORECASE):
                            ...
                            self.log.debug(
                                'Unsupported Boolean value: "{}"'.format(value)
                            )
                            cleaned_value = None
            ...
            for key, value in obj.items():
                ...
                if obj[
                    NODE_TYPE
                ] in self.schema.props.save_parent_id and is_parent_pointer(key):
                    header = key.split(".")
                    if len(header) > 2:
                        self.log.warning(
                            'Column header "{}" has multiple periods!'.format(key)
                        )
                        df_validation_result = self.update_field_validation_result(
                            df_validation_result,
                            file_name,
                            "",
                            "column_header_has_multiple_periods",
                            "warning",
                        )
                        if obj[NODE_TYPE] not in self.df_validation_dict.keys():
                            self.df_validation_dict[obj[NODE_TYPE]] = (
                                df_validation_result
                            )
                        else:
                            self.df_validation_dict[obj[NODE_TYPE]] = pd.concat(
                                [
                                    self.df_validation_dict[obj[NODE_TYPE]],
                                    df_validation_result,
                                ]
                    ...
                    if field_name in obj:
                        self.log.debug(
                            '"{}" field is in both current node and parent "{}", use {} instead !'.format(
                                key, parent, combined
                            )
                        )
                        field_name = combined
                ...
                for extra_prop_name, extra_value in self.schema.get_extra_props(
                    node_type, key, value
                ).items():
            ...
            if UUID not in obj2:
                ...
                if node_type:
                    if not id_value:
                        obj2[UUID] = self.schema.get_uuid_for_node(
                            node_type, self.get_signature(obj2)
                        )
                    elif id_field != UUID:
            ...
            df_validation_result = self.update_field_validation_result(
                df_validation_result, file_name, "", "type_column_missing", "error"
            )
            if OTHER not in self.df_validation_dict.keys():
                self.df_validation_dict[OTHER] = df_validation_result
            else:
                self.df_validation_dict[OTHER] = pd.concat(
                    [self.df_validation_dict[OTHER], df_validation_result]
                )
    ...
    def validate_cases_exist_in_file(self, file_name, max_violations):
        ...
        with self.driver.session() as session:
            ...
            with open(file_name, encoding=file_encoding) as in_file:
                self.log.info(
                    'Validating relationships in file "{}" ...'.format(file_name)
                )
                reader = csv.DictReader(in_file, delimiter="\t")
                line_num = 1
                validation_failed = False
                ...
                for org_obj in reader:
                    ...
                    if provided_parents > 0:
                        if len(relationships) == 0:
                            self.log.error(
                                "Invalid data at line {}: No parents found!".format(
                                    line_num
                                )
                            )
                            validation_failed = True
        ...
        with open(file_name, encoding=file_encoding) as in_file:
            ...
            for key in row.keys():
                if key not in parent_pointer:
                    try:
                        if (
                            key not in self.schema.get_props_for_node(row["type"])
                            and key != "type"
                        ):
                    ...
                    try:
                        if key.split(".")[1] not in self.schema.get_props_for_node(
                            key.split(".")[0]
                        ):
            ...
            if len(error_list) > 0:
                for error_field_name in error_list:
                    self.log.warning(
                        'Property: "{}" not found in data model'.format(
                            error_field_name
                        )
                    )
                    df_validation_result = self.update_field_validation_result(
                        ...
                        file_name,
                        error_field_name,
                        "property_not_found_in_model",
                        "warning",
                    )
            if len(parent_error_list) > 0:
                for parent_error_field_name in parent_error_list:
                    self.log.error(
                        'Parent pointer: "{}" not found in data model'.format(
                            parent_error_field_name
                        )
                    )
                    df_validation_result = self.update_field_validation_result(
                        df_validation_result,
                        file_name,
                        parent_error_field_name,
                        "parent_pointer_not_found_in_model",
                        "error",
                    )
                if len(df_validation_result) > 0:
                    if row[NODE_TYPE] not in self.df_validation_dict.keys():
                        ...
                        self.df_validation_dict[row[NODE_TYPE]] = pd.concat(
                            [
                                self.df_validation_dict[row[NODE_TYPE]],
                                df_validation_result,
                            ]
                        )
                self.log.error(
                    "Parent pointer not found in the data model, abort loading!"
                )
                return False
        if len(df_validation_result) > 0:
            if row[NODE_TYPE] not in self.df_validation_dict.keys():
    ...
    def update_field_validation_result(
        self, df_validation_result, file_name, error_field_name, reason, severity
    ):
        ...
        df_validation_result = pd.concat(
            [df_validation_result, tmp_df_validation_result_field]
        )
    ...
    def validate_file(self, file_name, max_violations, verbose):
        ...
        with open(file_name, encoding=file_encoding) as in_file:
            ...
            for org_obj in reader:
                ...
                if node_id:
                    if node_id in ids:
                        ...
                        ids[node_id] = {
                            "props": get_props_signature(props),
                            "lines": [str(line_num)],
                        }

                validate_result = self.schema.validate_node(
                    obj[NODE_TYPE], obj, verbose
                )
                try:
                    if len(validate_result["invalid_properties"]) > 0:
                        tmp_df_invalid = pd.DataFrame()
                        tmp_df_invalid["invalid_properties"] = validate_result[
                            "invalid_properties"
                        ]
                        tmp_df_invalid["invalid_values"] = validate_result[
                            "invalid_values"
                        ]
                        tmp_df_invalid["invalid_reason"] = validate_result[
                            "invalid_reason"
                        ]
                ...
                try:
                    if len(validate_result["missing_properties"]) > 0:
                        ...
                        tmp_df_missing["missing_properties"] = validate_result[
                            "missing_properties"
                        ]
                        tmp_df_missing["missing_reason"] = validate_result[
                            "missing_reason"
                        ]
                ...
                if not validate_result["result"] and not validate_result["warning"]:
                    for msg in validate_result["messages"]:
                        self.log.error(
                            'Invalid data at line {}: "{}"!'.format(line_num, msg)
                        )
                    validation_failed = True
                    violations += 1
                    if violations >= max_violations:
            ...
            # output the data validation result
            ...
            if len(df_invalid) > 0:
                ...
                df_invalid = (
                    df_invalid.explode("invalid_line_num")
                    .groupby(
                        [
                            "invalid_properties",
                            "invalid_values",
                ...
                tmp_df_validation_result_invalid["File Name"] = [
                    os.path.basename(file_name)
                ] * len(df_invalid)
                tmp_df_validation_result_invalid["Property"] = df_invalid[
                    "invalid_properties"
                ]
                tmp_df_validation_result_invalid["Value"] = df_invalid["invalid_values"]
                tmp_df_validation_result_invalid["Reason"] = df_invalid[
                    "invalid_reason"
                ]
                tmp_df_validation_result_invalid["Line Numbers"] = (
                    self.convert_line_num_list(list(df_invalid["invalid_line_num"]))
                )
                tmp_df_validation_result_invalid["Severity"] = ["error"] * len(
                    df_invalid
                )
                df_validation_result = pd.concat(
                    [df_validation_result, tmp_df_validation_result_invalid]
                )
            if len(df_missing) > 0:
                df_missing = df_missing.sort_values(by=["missing_properties"])
                df_missing = (
                    df_missing.explode("missing_line_num")
                    .groupby(["missing_properties", "missing_reason", "node_type"])[
                        "missing_line_num"
                    ]
                    .unique()
                    .reset_index()
                ...
                tmp_df_validation_result_missing["File Name"] = [
                    os.path.basename(file_name)
                ] * len(df_missing)
                tmp_df_validation_result_missing["Property"] = df_missing[
                    "missing_properties"
                ]
                tmp_df_validation_result_missing["Reason"] = df_missing[
                    "missing_reason"
                ]
                tmp_df_validation_result_missing["Line Numbers"] = (
                    self.convert_line_num_list(list(df_missing["missing_line_num"]))
                )
                tmp_df_validation_result_missing["Severity"] = ["error"] * len(
                    df_missing
                )
                df_validation_result = pd.concat(
                    [df_validation_result, tmp_df_validation_result_missing]
                )
            if len(df_duplicate_id) > 0:
                df_duplicate_id = (
                    df_duplicate_id.explode("duplicate_line_num")
                    .groupby(
                        [
                            "duplicate_id",
                            "duplicate_reason",
                ...
                tmp_df_validation_result_duplicate["File Name"] = [
                    os.path.basename(file_name)
                ] * len(df_duplicate_id)
                tmp_df_validation_result_duplicate["Property"] = df_duplicate_id[
                    "duplicate_id_field"
                ]
                tmp_df_validation_result_duplicate["Value"] = df_duplicate_id[
                    "duplicate_id"
                ]
                tmp_df_validation_result_duplicate["Reason"] = df_duplicate_id[
                    "duplicate_reason"
                ]
                tmp_df_validation_result_duplicate["Line Numbers"] = (
                    self.convert_line_num_list(
                        list(df_duplicate_id["duplicate_line_num"])
                    )
                )
                tmp_df_validation_result_duplicate["Severity"] = ["error"] * len(
                    df_duplicate_id
                )
                df_validation_result = pd.concat(
                    [df_validation_result, tmp_df_validation_result_duplicate]
                )
            if len(df_validation_result) > 0:
                if obj[NODE_TYPE] not in self.df_validation_dict.keys():
    ...
    def get_upsert_statement(self, node_type, id_field, obj):
        ...
        statement += (
            " ON CREATE "
            + "SET n.{} = datetime(), ".format(CREATED)
            + " ,".join(prop_stmts)
        )
        statement += (
            " ON MATCH "
            + "SET n.{} = datetime(), ".format(UPDATED)
            + " ,".join(prop_stmts)
        )
        return statement
    ...
    def get_children_with_single_parent(self, session, node):
        ...
        statement = "MATCH (n:{0} {{ {1}: ${1} }})<--(m)".format(
            node_type, self.schema.get_id_field(node)
        )
    ...
    def delete_single_node(self, session, node):
        node_type = node[NODE_TYPE]
        statement = "MATCH (n:{0} {{ {1}: ${1} }}) detach delete n".format(
            node_type, self.schema.get_id_field(node)
        )
        result = session.run(statement, node)
        nodes_deleted = result.consume().counters.nodes_deleted
        self.nodes_deleted += nodes_deleted
        self.nodes_deleted_stat[node_type] = (
            self.nodes_deleted_stat.get(node_type, 0) + nodes_deleted
        )
    ...
    def load_nodes(self, session, file_name, loading_mode, split=False):
        ...
        with open(file_name, encoding=file_encoding) as in_file:
            ...
            for org_obj in reader:
                ...
                if loading_mode == UPSERT_MODE:
                    ...
                    if self.node_exists(tx, node_type, id_field, node_id):
                        raise Exception(
                            "Line: {}: Node (:{} {{ {}: {} }}) exists! Abort loading!".format(
                                line_num, node_type, id_field, node_id
                            )
                        )
                    else:
                ...
                if loading_mode != DELETE_MODE:
                    ...
                    if (
                        ...
                        and result.consume().counters._contains_updates
                    ):
                        update_count = 1
                    ...
                    self.nodes_stat[node_type] = (
                        self.nodes_stat.get(node_type, 0) + count
                    )
                    self.nodes_stat_updated[node_type] = (
                        self.nodes_stat_updated.get(node_type, 0) + update_count
                    )
            ...
            if loading_mode == DELETE_MODE:
                ...
                self.log.info(
                    "{} (:{}) node(s) loaded".format(nodes_created, node_type)
                )
                self.log.info(
                    "{} (:{}) node(s) updated".format(nodes_updated, node_type)
                )
        ...
        for key, value in obj.items():
            if is_parent_pointer(key):
                ...
                if not self.node_exists(session, other_node, other_id, value):
                    ...
                    if create_intermediate_node:
                        for plugin in self.plugins:
                            if plugin.should_run(other_node, MISSING_PARENT):
                                create_parent = True
                                if plugin.create_node(
                                    session, line_num, other_node, value, obj
                                ):
                                    int_node_created += 1
                                    relationships.append(
                                        {
                                            PARENT_TYPE: other_node,
                                            PARENT_ID_FIELD: other_id,
                                            PARENT_ID: value,
                                    ...
                                    self.log.error(
                                        "Line: {}: Could not create {} node automatically!".format(
                                            line_num, other_node
                                        )
                                    )
                    else:
                        self.log.warning(
                            'Line: {}: Parent node (:{} {{{}: "{}"}} not found in DB!'.format(
                                line_num, other_node, other_id, value
                            )
                        )
                    ...
                    if multiplier == ONE_TO_ONE and self.parent_already_has_child(
                        session,
                        node_type,
                        obj,
                        relationship_name,
                        ...
                        self.log.error(
                            "Line: {}: one_to_one relationship failed, parent already has a child!".format(
                                line_num
                            )
                        )
                    else:
    ...
    def parent_already_has_child(
        ...
        statement = "MATCH (n:{})-[r:{}]->(m:{} {{ {}: $parent_id }}) return n".format(
            node_type, relationship_name, parent_type, parent_id_field
        )
        result = session.run(statement, {"parent_id": parent_id})
        if result:
            child = result.single()
            if child:
                find_current_node_statement = (
                    "MATCH (n:{0} {{ {1}: ${1} }}) return n".format(
                        node_type, self.schema.get_id_field(node)
                    )
                )
    ...
    def has_existing_relationship(
        self, session, node_type, node, relationship, count_same_parent=False
    ):
        relationship_name = relationship[RELATIONSHIP_TYPE]
        parent_type = relationship[PARENT_TYPE]
        parent_id_field = relationship[PARENT_ID_FIELD]

        base_statement = "MATCH (n:{0} {{ {1}: ${1} }})-[r:{2}]->(m:{3})".format(
            node_type, self.schema.get_id_field(node), relationship_name, parent_type
        )
        ...
        if result:
            ...
            if old_parent:
                if count_same_parent:
                    ...
                    if old_parent_id != relationship[PARENT_ID]:
                        self.log.warning(
                            "Old parent is different from new parent, delete relationship to old parent:"
                            + ' (:{} {{ {}: "{}" }})!'.format(
                                parent_type, parent_id_field, old_parent_id
                            )
                        )
                        del_statement = base_statement + " delete r"
                        return del_statement
            ...
            self.log.error(
                "Remove old relationship failed: Query old relationship failed!"
            )

        return False

    def remove_old_relationship(self, session, node_type, node, relationship):
        del_statement = self.has_existing_relationship(
            session, node_type, node, relationship
        )
        if del_statement:
        ...
        with open(file_name, encoding=file_encoding) as in_file:
            ...
            for org_obj in reader:
                ...
                if provided_parents > 0:
                    if len(relationships) == 0:
                        raise Exception(
                            "Line: {}: No parents found, abort loading!".format(
                                line_num
                            )
                        )
                    for relationship in relationships:
                        ...
                        if multiplier in [DEFAULT_MULTIPLIER, ONE_TO_ONE]:
                            if loading_mode == UPSERT_MODE:
                                self.remove_old_relationship(
                                    tx, node_type, obj, relationship
                                )
                            elif loading_mode == NEW_MODE:
                                if self.has_existing_relationship(
                                    tx, node_type, obj, relationship, True
                                ):
                                    raise Exception(
                                        "Line: {}: Relationship already exists, abort loading!".format(
                                            line_num
                                        )
                                    )
                            else:
                                raise Exception(
                                    "Wrong loading_mode: {}".format(loading_mode)
                                )
                        else:
                            self.log.debug(
                                "Multiplier: {}, no action needed!".format(multiplier)
                            )
                        prop_statement = ", ".join(
                            self.get_relationship_prop_statements(properties)
                        )
                        statement = "MATCH (m:{0} {{ {1}: $__parentID__ }})".format(
                            parent_node, parent_id_field
                        )
                        statement += " MATCH (n:{0} {{ {1}: ${1} }})".format(
                            node_type, self.schema.get_id_field(obj)
                        )
                        statement += " MERGE (n)-[r:{}]->(m)".format(relationship_name)
                        statement += " ON CREATE SET r.{} = datetime()".format(CREATED)
                        statement += (
                            ", {}".format(prop_statement) if prop_statement else ""
                        )
                        ...
                        result = tx.run(
                            statement, {**obj, "__parentID__": parent_id, **properties}
                        )
                        count = result.consume().counters.relationships_created
                        self.relationships_created += count
                        relationship_pattern = "(:{})->[:{}]->(:{})".format(
                            node_type, relationship_name, parent_node
                        )
                        relationships_created[relationship_pattern] = (
                            relationships_created.get(relationship_pattern, 0) + count
                        )
                        self.relationships_stat[relationship_name] = (
                            self.relationships_stat.get(relationship_name, 0) + count
                        )
                    for plugin in self.plugins:
                        if plugin.should_run(node_type, NODE_LOADED):
                            if plugin.create_node(
                                session=tx, line_num=line_num, src=obj
                            ):
                                int_nodes_created += 1
                # commit and restart a transaction when batch size reached
                if split and transaction_counter >= BATCH_SIZE:
            ...
            if provided_parents == 0:
                self.log.warning(
                    "there is no parent mapping columns in the node {}".format(
                        node_type
                    )
                )
            ...
            if int_nodes_created > 0:
                self.log.info(
                    "{} intermediate node(s) loaded".format(int_nodes_created)
                )
    ...
    def wipe_db(self, session, split=False):
        if split:
            ...
            self.log.info(
                "{} relationships deleted!".format(self.relationships_deleted)
            )
    ...
    def create_index(self, node_name, node_property, existing, session):
        ...
        if index_tuple not in existing:
            ...
            self.log.info(
                'Index created for "{}" on property "{}"'.format(
                    node_name, node_property
                )
            )
