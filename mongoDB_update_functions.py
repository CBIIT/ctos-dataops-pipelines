            ...
            if item[DATA_COMMONS] == data_commons:
                if item.get(NODE_TYPE) and item.get(NODE_TYPE) == node:
                    if item.get(PROPS).get(old_parent_id_field):
                        old_id = str(item[PROPS][old_parent_id_field])
                        new_id = update_dict.get(old_id)
                        if new_id and old_id != new_id:
                            if not item.get(HISTORY):
                                item = add_initial_history_item(item)
                            item[PROPS][new_parent_id_field] = new_id
                            item[NODE_ID] = new_id
                    ...
                    if item.get(PARENTS):
                        for index, parent in enumerate(item[PARENTS]):
                            if parent.get(PARENT_TYPE) and parent.get(PARENT_TYPE) == node:
                                old_parent_id_value = str(parent[PARENT_ID_VALUE])
                                new_parent_id_value = update_dict.get(old_parent_id_value)
                                if new_parent_id_value and old_parent_id_value != new_parent_id_value:
                                    if not item.get(HISTORY):
                                        item = add_initial_history_item(item)
...
if __name__ == "__main__":
    ...
    with open("config/mongodb_config_update_test.yaml", "r") as f:
