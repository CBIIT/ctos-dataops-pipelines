...
def icdc_dataloading(argList: dict):
    ...
    if plugins:
        raise NotImplementedError('Plugin support has not been ported yet; pass an empty plugins list.')

    if split_transactions and no_backup:
        raise Exception('split_transactions and no_backup cannot both be enabled, a backup is required '
    ...
    if s3_folder:
        if not os.path.exists(dataset):
            ...
            if len(exist_files) > 0:
                raise Exception(f'Folder: "{dataset}" is not empty, please empty it first')
        if not s3_bucket:
    ...
    try:
        ...
        if file_list:
            ...
            load_result = loader.load(
                file_list, cheat_mode, dry_run, mode, wipe_db, max_violations, temp_folder, verbose,
                split=split_transactions, no_backup=no_backup, backup_folder=backup_folder, neo4j_uri=neo4j_uri,
            )
            ...
            if load_result is False:
                if loader.validation_result_file_key:
                    ...
                    logger.error('Data loading failed, validation result zip file was created at {}'.format(zip_file_key))
                else:
                ...
                with zipfile.ZipFile(zip_file_key, 'w') as zipf:
                    zipf.write(log_file, os.path.basename(log_file))
                logger.info('Data loading succeeded, zip file was created at {}'.format(zip_file_key))
        else:
    ...
    if upload_log_dir and zip_file_key:
        try:
            ...
            logger.debug(e)
            logger.exception('Uploading log file failed! Check debug log for detailed information')
...
def main():
    ...
    parser.add_argument('config_file', type=argparse.FileType('r'), help='Loader configuration YAML')
