    ...
    try:
        ...
        for i, obj in enumerate(files, 1):
            ...
            if i % 10 == 0 or i == len(files):
                log.info(f'Downloaded {file_count} of {len(files)} files ({format_file_size(total_size)} total)')

        log.info(f'Download complete: {file_count} files, {format_file_size(total_size)}')
...
def create_zip_archive(source_dir, zip_path, log):
    ...
    try:
        ...
        log.info(f'Zip archive created: {file_count} files, {format_file_size(zip_size)}')
        return zip_size
...
def s3_archiving(source_bucket, source_prefix, target_bucket, target_prefix, zip_filename):
    """
    ...
    temp_base_dir = '/usr/local/data' if os.path.exists('/usr/local/data') and os.access('/usr/local/data', os.W_OK) else '/tmp'
