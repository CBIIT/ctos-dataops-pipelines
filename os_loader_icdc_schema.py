...
class ICDC_Schema:
    def __init__(self, yaml_files, props):
        ...
        if not yaml_files:
            raise Exception(
                "File list is empty,could not initialize ICDC_Schema object!"
            )
        else:
        ...
        if END_POINTS in desc:
            for end_points in desc[END_POINTS]:
                ...
                if MULTIPLIER in end_points:
                    ...
                    self.log.debug(
                        'End point multiplier: "{}" overriding relationship multiplier: "{}"'.format(
                ...
                if dest in self.nodes:
                    self.add_relationship_to_node(
                        dest, actual_multiplier, name, src, True
                    )
    ...
    def add_relationship_to_node(
        self, name, multiplier, relationship, other_node, dest=False
    ):
        node = self.nodes[name]
        if multiplier == "many_to_one":
            if dest:
                node[PROPERTIES][self.plural(other_node)] = {
                    PROP_TYPE: '[{}] @relation(name:"{}", direction:IN)'.format(
                        other_node, relationship
                    )
                }
            else:
                node[PROPERTIES][other_node] = {
                    PROP_TYPE: '{} @relation(name:"{}", direction:OUT)'.format(
                        other_node, relationship
                    )
                }
            ...
            self.log.warning(
                'Unsupported relationship multiplier: "{}"'.format(multiplier)
            )
        ...
        if name in self.org_schema[PROP_DEFINITIONS]:
            ...
            if key:
                ...
                if isinstance(prop_desc, str):
                    ...
                    self.log.debug(
                        'Property type: "{}" not supported, use default type: "{}"'.format(
    ...
    def get_extra_props(self, node_name, name, value):
        ...
        if prop and HAS_UNIT in prop and prop[HAS_UNIT]:
            ...
            results[self.get_unit_property_name(name)] = self.get_default_unit(
                node_name, name
            )
            ...
            results[self.get_unit_property_name(org_prop_name)] = self.get_default_unit(
                node_name, name
            )
        return results
                ...
                if isinstance(prop_desc, dict):
                    if UNITS in prop_desc:
                        ...
                        if units:
                            enum = set(units)
                            unit_prop_name = self.get_unit_property_name(name)
                            results[unit_prop_name] = {
                                PROP_TYPE: DEFAULT_TYPE,
                                ENUM: enum,
                                DEFAULT_VALUE: units[0],
                            }
                            org_prop_name = self.get_original_value_property_name(name)
                            org_unit_prop_name = self.get_unit_property_name(
                                org_prop_name
                            )
                            results[org_prop_name] = prop_type
                            results[org_unit_prop_name] = {
                                PROP_TYPE: DEFAULT_TYPE,
                                ENUM: enum,
                                DEFAULT_VALUE: units[0],
                            }
    ...
    def validate_node(self, model_type, obj, verbose):
        ...
        if not model_type or model_type not in self.nodes:
            return {
                "result": False,
                "messages": [
                    'Node type: "{}" not found in data model'.format(model_type)
                ],
                "warning": False,
            }
        if not obj:
        ...
        for prop in self.nodes[model_type].get(REQUIRED, set()):
            if prop not in obj:
                ...
                result["messages"].append(
                    'Missing required property: "{}"!'.format(prop)
                )
                ...
                result["result"] = False
                result["messages"].append(
                    'Required property: "{}" is empty!'.format(prop)
                )
                result["missing_properties"].append(prop)
        ...
        for key, value in obj.items():
            if key == NODE_TYPE:
                ...
                if rel_type not in self.relationship_props:
                    result["result"] = False
                    result["messages"].append(
                        f'Relationship "{rel_type}" does NOT exist in data model!'
                    )
                    continue
                elif rel_prop not in self.relationship_props[rel_type][PROPERTIES]:
                    result["result"] = False
                    result["messages"].append(
                        f'Property "{rel_prop}" does NOT exist in relationship "{rel_type}"!'
                    )
                    continue

                prop_type = self.relationship_props[rel_type][PROPERTIES][rel_prop]
                type_validation_result, error_type = self._validate_type(
                    prop_type, value
                )
                if not type_validation_result:
                    ...
                    if not verbose:
                        if error_type == "non_permissive_value":
                            result["messages"].append(
                                'Property: "{}":"{}" is not in permissible value list!'.format(
                                    rel_prop, value
                                )
                            )
                        elif error_type == "wrong_type":
                            result["messages"].append(
                                'Property: "{}":"{}" is in wrong type!'.format(
                                    rel_prop, value
                                )
                            )
                    else:
                        result["messages"].append(
                            'Property: "{}":"{}" is not a valid "{}" type!'.format(
                                rel_prop, value, prop_type
                            )
                        )
    ...
    def _validate_type(self, model_type, str_value):
        ...
        if model_type[PROP_TYPE] == "Float":
            ...
            if (
                str_value
                and not re.match(r"\byes\b|\btrue\b", str_value, re.IGNORECASE)
            ...
            for item in self.get_list_values(str_value):
                if ENUM in model_type[ITEM_TYPE]:
                    validation_result, error_type = self._validate_type(
                        model_type[ITEM_TYPE], item
                    )
    ...
    def get_relationship(self, src, dest):
        if src in self.relationships:
            ...
            if relationships and dest in relationships:
                ...
                self.log.error(
                    'No relationships found for "{}"-->"{}"'.format(src, dest)
                )
                return None
        else:
    ...
    def map_type(self, type_name):
        ...
        if type_name in mapping:
            ...
            self.log.debug(
                'Type: "{}" has no mapping, use default type: "{}"'.format(
    ...
    def get_id_field(self, obj):
        if NODE_TYPE not in obj:
            self.log.error(
                'get_id_field: there is no "{}" field in node, can\'t retrieve id!'.format(
                    NODE_TYPE
                )
            )
            return None
